...
bool Motor::setOperatingMode(OperatingMode mode)
{
    ...
    for (int i = 0; i < DXL_ID_CNT; i++)
    {
        ...
        {
            if (ret)
                ret = _dxl.torqueOff(DXL_ID_LIST[i]);
            if (ret)
                ret = _dxl.setOperatingMode(DXL_ID_LIST[i], mode);
        }
    }
    if (ret)
        ret = _dxl.torqueOn(BROADCAST_ID);
    return ret;
}
...
bool Motor::setPID(int id, int p, int i, int d)
{
    ...
    if (ret)
        ret = _dxl.writeControlTableItem(ControlTableItem::POSITION_I_GAIN, id, i);
    if (ret)
        ret = _dxl.writeControlTableItem(ControlTableItem::POSITION_D_GAIN, id, d);
    return ret;
}
...
bool Motor::move(int val)
{
    ...
    if (val <= INIT_ROTATION)
    {
        val = 0;
        val2 = 180;
        _servo_position = INIT_ROTATION;
    }else{
        val = 180;
        val2 = 0;
        _servo_position = 2046 + INIT_ROTATION;
    }
